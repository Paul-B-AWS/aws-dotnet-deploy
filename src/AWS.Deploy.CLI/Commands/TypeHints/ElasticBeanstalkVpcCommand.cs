...
namespace AWS.Deploy.CLI.Commands.TypeHints
{
    ...
    public class ElasticBeanstalkVpcCommand : ITypeHintCommand
    {
        ...
        public async Task<object> Execute(Recommendation recommendation, OptionSettingItem optionSetting)
        {
            ...
            var loadBalancerSubnets = new SortedSet<string>();
            var loadBalancerSubnetsOptionSetting = optionSetting.ChildOptionSettings.First(x => x.Id.Equals("LoadBalancerSubnets"));

            var environmentTypeOptionSetting = _optionSettingHandler.GetOptionSetting(recommendation, "EnvironmentType");
            var environmentTypeOptionSettingValue = _optionSettingHandler.GetOptionSettingValue(recommendation, environmentTypeOptionSetting);
            if (environmentTypeOptionSettingValue.Equals("LoadBalanced"))
            {
                _toolInteractiveService.WriteLine($"{loadBalancerSubnetsOptionSetting.Id}:");
                _toolInteractiveService.WriteLine(loadBalancerSubnetsOptionSetting.Description);
                loadBalancerSubnets = _consoleUtilities.AskUserForList<Subnet>(userInputConfigurationSubnets, availableSubnets, loadBalancerSubnetsOptionSetting, recommendation);
            }
